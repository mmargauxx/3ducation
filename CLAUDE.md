...
The homepage (`front-page.html`) is a brochure — hero → **pillars** (three-way split) → **audience-split** (B2B/B2C: Voor Scholen vs Voor Thuis, two-up, magenta/cyan, followed by a `.social-proof-placeholder` reserved for workshop/classroom photos) → partners → webshop-CTA. (The old `offering.php` "Wat we doen" pattern was removed as redundant to the pillars.) The site is organised around **three pillars**, each a slug-mapped page (`page-<slug>.html` + patterns, registered in `theme.json` `customTemplates`):
