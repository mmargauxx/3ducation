...
- **Naming:** PHP prefix is `threeducation_` (note: the enqueue callback is misspelled `threducation_enqueue_assets` — keep it consistent if referenced). The **text domain and pattern-category slug stay `3ducation`** (lowercase); do not rename them. Version constant `THREEDUCATION_VERSION` (currently `0.15.4`) also lives in `style.css` — bump both together, and it doubles as the asset cache-buster.
