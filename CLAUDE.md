...
- **Pattern cache:** after adding OR editing any `patterns/*.php`, `wp cache flush` alone is NOT enough — the block-pattern list is cached separately. Run `./node_modules/.bin/wp-env run cli wp eval 'wp_clean_themes_cache(); wp_cache_flush();'` or the pattern renders empty/stale. Template HTML edits (`templates/*.html`) are read from disk — no cache clear needed. **This bites during local editing only**, where the theme version stays put: since WP 6.4 the cache is keyed on the theme's `Version:` header (`WP_Theme::get_pattern_cache()`, `wp-includes/class-wp-theme.php`), so a released version bump invalidates it by itself — see the deploy note in `GO-LIVE.md`. To stop it biting locally at all, add `"WP_DEVELOPMENT_MODE": "theme"` to `config` in `.wp-env.json`; core then skips the pattern cache entirely (`WP_Theme::get_block_patterns()` checks `wp_is_development_mode( 'theme' )`).
...
- **Cal.com booking links** (`Settings → Boekingslinks`) — the workshop + feestje booking URLs, editable by the client, stored in option `threeducation_calcom_urls` (empty field = fall back to the hardcoded default in `threeducation_calcom_url_defaults()`; the filter `threeducation_calcom_url` still wins). `threeducation_calcom_popup_path()` decides per link whether the button gets the popup embed (`assets/cal-embed.js`) or stays a plain new-tab link: only a cal.com host with a `<user>/<event-slug>` path gets the popup. The workshop runs on Cal.com's newer fixed-date **Events** pages (`cal.com/<slug>`), so it is new-tab by design — and its URL changes per session.
- **Intake forms** — the three patterns (`contact-form`, `service-intake`, `workshops-intake`) POST to an `admin-post.php` handler (`threeducation_handle_intake`); `threeducation_intake_config()` is the field map, `threeducation_intake_hidden_fields()`/`_notice()` render the nonce+honeypot and the success/error banner. Validates + `wp_mail()`s to `info@3ducation.be` (filter `threeducation_intake_recipient`); the service form attaches images/videos. Needs an SMTP plugin on live for delivery. No form plugin — the hand-styled markup stays.

### Content flow

The homepage (`front-page.html`) is a brochure — hero → **pillars** (three-way split) → **audience-split** (B2B/B2C: Voor Scholen vs Voor Thuis, two-up, color-coded per pillar — scholen = cyan (→educatie), thuis = magenta (→webshop) — followed by a `.social-proof-placeholder` reserved for workshop/classroom photos) → partners → webshop-CTA. (The old `offering.php` "Wat we doen" pattern was removed as redundant to the pillars.) The site is organised around **three pillars**, each a slug-mapped page (`page-<slug>.html` + patterns, registered in `theme.json` `customTemplates`):
